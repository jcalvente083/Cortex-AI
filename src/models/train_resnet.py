"""
train_resnet.py — Fine-tuning ResNet18 sobre mel-espectrogramas (4 actividades de habla).
...
Salidas:
  models/resnet/ResNet18/{run}/{actividad}/    -> model.pth, threshold.pkl
  reports/resnet/ResNet18/{run}/{actividad}/   -> 6 figuras por modelo
  reports/resnet/ResNet18/{run}/comparison_4models.png
  logs/resnet/ResNet18/train_{run}_{timestamp}.log

Uso:
  uv run python -m src.models.train_resnet --run baseline
  uv run python -m src.models.train_resnet --run specaugment
  uv run python -m src.models.train_resnet --run specaugment_freeze --freeze
  uv run python -m src.models.train_resnet --run age_matched --age-match
  uv run python -m src.models.train_resnet --run age_matched_freeze --age-match --freeze
  uv run python -m src.models.train_resnet --run run2 --epochs 40 --batch-size 16
"""
...
REPORTS_BASE = Path("reports/resnet/ResNet18")
MODELS_BASE  = Path("models/resnet/ResNet18")
LOG_DIR      = Path("logs/resnet/ResNet18")
...
LR           = 1e-4
WEIGHT_DECAY = 1e-4
PATIENCE     = 7    # early stopping sobre val_loss
...
class AudioDataset(Dataset):
    """
    Pre-carga mel-espectrogramas en memoria al inicializar.
    Cada elemento es un frame de 0.4s como array (n_mels, time_steps).
    El resize a 224x224 se aplica en el transform del DataLoader.
    """

    def __init__(self, df_meta: pd.DataFrame, transform=None, training: bool = False):
        self.transform   = transform
        self.training    = training
        self.frames      = []        # np.float32 (n_mels, time_steps)
        self.targets     = []        # int
        self.patient_ids = []        # str
        ...
        for _, row in tqdm(df_meta.iterrows(), total=len(df_meta),
            ...
            try:
                ...
                # y_framed: (frame_len, n_frames) — melspectrogram acepta batched input
    ...
    def __getitem__(self, idx):
        tensor = torch.from_numpy(self.frames[idx]).unsqueeze(0)  # (1, n_mels, time)
        if self.transform:
...
class ResNet18Trainer:
    ...
    def __init__(self, activity_name: str, epochs: int = EPOCHS, batch_size: int = BATCH_SIZE,
        ...
        self._sample_specs = {}   # {0: [spec_array, ...], 1: [spec_array, ...]}
    ...
    def fit(self, df_train: pd.DataFrame, df_holdout: pd.DataFrame) -> "ResNet18Trainer":
        n_pac_tr = df_train['ID_Paciente'].nunique()
        n_pac_ho = df_holdout['ID_Paciente'].nunique()

        print(f"\n{'='*60}")
        print(f"  ENTRENAMIENTO — ResNet18 — [{self.activity_name.upper()}]")
        print(f"{'='*60}")
        ...
        # ── Umbral de Youden (nivel paciente, val interna) ───────────────
    ...
    def _plot_confusion(self):
        ...
        fig.suptitle(f"Matriz de Confusion — ResNet18 [{self.activity_name}]",
    ...
    def _plot_roc(self):
        ...
        ax.set_title(f"Curva ROC — ResNet18 [{self.activity_name}]", fontweight="bold")
    ...
    def _plot_probability_distribution(self):
        ...
        fig.suptitle(
            f"Distribucion de Probabilidades — ResNet18 [{self.activity_name}]",
    ...
    def _plot_spectrogram_examples(self):
        ...
        fig.suptitle(
            f"Ejemplos de Mel-Espectrograma — ResNet18 [{self.activity_name}]",
            fontweight="bold", fontsize=13,
        )
...
def plot_comparison(results: dict):
    ...
    ax.set_title("Comparacion ResNet18 — Holdout externo (4 actividades)",
                 fontweight="bold", fontsize=13)
    ax.set_xticks(x)
...
def main():
    ...
    parser.add_argument('--freeze',     action='store_true',
                        help='Congela conv1, bn1, layer1, layer2 (~3M params entrenables)')
    ...
    with Tee(log_path):
        ...
        print(f"{'='*60}")
        print("  Cortex-AI — Entrenamiento ResNet18 (4 actividades)")
        print(f"  Inicio  : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"  Epochs  : {args.epochs} | Batch: {args.batch_size} | LR: {LR}")
        print(f"  Device  : {'CUDA' if torch.cuda.is_available() else 'CPU'}")
        print(f"{'='*60}")

        # ── Cargar y alinear metadatos con el split paciente-nivel ───────
        ...
        if results:
            ...
            save_run_json("ResNet18", args.run, results, thresholds)
