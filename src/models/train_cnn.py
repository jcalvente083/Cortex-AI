"""
train_cnn.py — CNN ligera entrenada desde cero sobre mel-espectrogramas (4 actividades).

Arquitectura CortexCNN: 5 capas convolucionales, sin pesos preentrenados (~980K params).
Sirve como ablación frente a ResNet18 (pretrained ImageNet) para justificar el uso
de transfer learning con datasets clínicos pequeños.
...
Salidas:
  models/cnn/CortexCNN/{run}/{actividad}/    -> model.pth, threshold.pkl
  reports/cnn/CortexCNN/{run}/{actividad}/   -> 6 figuras por modelo
  reports/cnn/CortexCNN/{run}/comparison_4models.png
  logs/cnn/CortexCNN/train_{run}_{timestamp}.log

Uso:
  uv run python -m src.models.train_cnn --run baseline
  uv run python -m src.models.train_cnn --run age_matched --age-match
  uv run python -m src.models.train_cnn --run run2 --epochs 60 --batch-size 16
"""
...
REPORTS_BASE = Path("reports/cnn/CortexCNN")
MODELS_BASE  = Path("models/cnn/CortexCNN")
LOG_DIR      = Path("logs/cnn/CortexCNN")

# =============================================================================
# HIPERPARAMETROS
# =============================================================================
RESIZE       = 224
EPOCHS       = 50   # más épocas que ResNet18 (sin pretrain necesita más)
BATCH_SIZE   = 32
LR           = 5e-4  # más alto que ResNet18 (1e-4): entrenamiento desde cero
WEIGHT_DECAY = 1e-4
PATIENCE     = 10   # más paciencia para dar margen al entrenamiento desde cero
LR_PATIENCE  = 4
...
# =============================================================================
# ARQUITECTURA — CortexCNN
# =============================================================================
class CortexCNN(nn.Module):
    """
    CNN ligera de 5 bloques convolucionales entrenada desde cero.
    ~980K parametros entrenables (vs ~11M de ResNet18 / ~3M con freeze).

    Diseñada como ablación para demostrar el valor del transfer learning
    con datasets clínicos de tamaño reducido (N~165 pacientes).
    ...
    def __init__(self, num_classes: int = 2, dropout: float = 0.5):
        super().__init__()
        self.block1 = self._conv_block(1,   32)   # (B,  32, 224, 224)
        self.block2 = self._conv_block(32,  64)   # (B,  64, 112, 112)
        self.block3 = self._conv_block(64,  128)  # (B, 128,  56,  56)
        self.block4 = self._conv_block(128, 256)  # (B, 256,  28,  28)
        self.block5 = self._conv_block(256, 256)  # (B, 256,  14,  14)
...
class AudioDataset(Dataset):
    ...
    def __init__(self, df_meta: pd.DataFrame, transform=None, training: bool = False):
        ...
        for _, row in tqdm(df_meta.iterrows(), total=len(df_meta),
                           desc="Cargando espectrogramas", leave=False):
            try:
...
class CortexCNNTrainer:
    ...
    def _build_model(self) -> nn.Module:
        return CortexCNN(num_classes=2, dropout=0.5).to(self.device)
    ...
    def fit(self, df_train: pd.DataFrame, df_holdout: pd.DataFrame) -> "CortexCNNTrainer":
        n_pac_tr = df_train['ID_Paciente'].nunique()
        n_pac_ho = df_holdout['ID_Paciente'].nunique()

        print(f"\n{'='*60}")
        print(f"  ENTRENAMIENTO — CortexCNN — [{self.activity_name.upper()}]")
        print(f"{'='*60}")
    ...
    def _plot_confusion(self):
        ...
        fig.suptitle(f"Matriz de Confusion — CortexCNN [{self.activity_name}]",
    ...
    def _plot_roc(self):
        ...
        ax.set_title(f"Curva ROC — CortexCNN [{self.activity_name}]", fontweight="bold")
    ...
    def _plot_probability_distribution(self):
        ...
        fig.suptitle(
            f"Distribucion de Probabilidades — CortexCNN [{self.activity_name}]",
    ...
    def _plot_spectrogram_examples(self):
        ...
        fig.suptitle(
            f"Ejemplos de Mel-Espectrograma — CortexCNN [{self.activity_name}]",
            fontweight="bold", fontsize=13,
        )
...
def plot_comparison(results: dict):
    ...
    ax.set_title("Comparacion CortexCNN — Holdout externo (4 actividades)",
                 fontweight="bold", fontsize=13)
    ax.set_xticks(x)
...
def main():
    ...
    parser.add_argument('--age-match',  action='store_true',
                        help='Age-matching 1:1 (±5 años) en NeuroVoz')
    ...
    with Tee(log_path):
        ...
        print(f"{'='*60}")
        print("  Cortex-AI — Entrenamiento CortexCNN (4 actividades)")
        ...
        if results:
            ...
            save_run_json("CortexCNN", args.run, results, thresholds)
